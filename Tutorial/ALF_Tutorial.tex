...
\begin{itemize}
	...
	\item The measurement of a equal-time correlation function consists of two separate parts, the connected one given by the first term in Eq.~\ref{eq:new_obs}, and the background, the second term.
...
Finally, it is straight forward to implement the time-displaced version of this correlation function, following essentially the same steps as described above. The observable is now stored in \texttt{Obs\_tau}, the measurements are taken in \texttt{ObserT}, and you can find the implementation in the solutions to this exercise as well.
