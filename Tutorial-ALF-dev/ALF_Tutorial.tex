...
\section{Installing ALF+pyALF}
\subsection{Installing ALF Requirements}
...
\begin{lstlisting}[style=bash]
sudo yum install make gfortran blas-devel lapack-devel \
          python3 openmpi-devel g++ curl zlib-devel git
\end{lstlisting}

\textbf{OpenSuSE/SLES:}
\begin{lstlisting}[style=bash]
sudo zypper install make gcc-fortran lapack-devel \
             python3 openmpi-devel gcc-c++ curl zlib-devel git
\end{lstlisting}
...
\subsection*{MacOS}
For MacOS, Xcode and the Apple developer tools should be installed.
...
If you do not yet have python environment with working pip, we recommend 
\href{https://github.com/conda-forge/miniforge}{miniforge} or
\href{https://www.anaconda.com/download/success}{Anaconda}.
}:
\begin{lstlisting}[style=bash]
pip install pyALF
\end{lstlisting}

To use the Jupyter Notebooks utilized in the following parts of this tutorial, Jupyter is needed:
\begin{lstlisting}[style=bash]
pip install jupyter
\end{lstlisting}
...
\subsection{Alternative: Ready-to-use container image}

For a ready-to-use environment, one can use the Docker image \href{https://hub.docker.com/r/alfcollaboration/jupyter-pyalf-full}{alfcollaboration/jupyter-pyalf-full}, which has the above mentioned dependencies, ALF and pyALF installed. With a suitable container runtime e.g. \href{https://www.docker.com/}{Docker} or \href{https://podman.io/}{Podman}, it can be used run ALF and pyALF without any further setup. It is derived from the Jupyter Docker Stacks, therefore \href{https://jupyter-docker-stacks.readthedocs.io}{this documentation} applies. For example, one could run a container like this:
...
\begin{itemize}
   \item The \href{https://docs.docker.com/reference/cli/docker/container/run/#publish}{-p} flag is used to expose port 8888
   and you can access a JupyterLab running within the container by navigating to \url{http://localhost:8888/lab?token=<token>}
   with you browser, where \texttt{<token>} has to be replaced by the token echoed to the terminal on startup.
   \item The \href{https://docs.docker.com/reference/cli/docker/container/run/#volume}{-v} flag mounts the
   current working directory to \texttt{/home/jovyan/work} within the container, allowing to work on
   the same data in- and outside of the container.
   \item The \href{https://docs.docker.com/reference/cli/docker/container/run/#rm}{--rm} flag instructs Docker
   to automatically remove the container after it exits, avoiding cluttering up the system with unused containers.
   \item The \href{https://docs.docker.com/reference/cli/docker/container/run/#interactive}{-i} and
   \href{https://docs.docker.com/reference/cli/docker/container/run/#tty}{-t} flags keep the
...
launches a container, which executes \texttt{minimal\_ALF\_run}, saving the results in the current working directory and removing the container right after that.
